...
class Berserk(Hero):
    ...
    def apply_super_ability(self, boss, heroes):
        if self.health - boss.damage:
            self.damage = self.damage + (boss.damage / 100)
        else:
...
def start():
    boss = Boss("Lord", randint(700, 30000), 150)
    warrior = Warrior("Curry", randint(150, 150), 10)
    medic_1 = Medic("Doctor", randint(100, 110), 5, 15)
    ...
    tricky = TrickyBastard("Tricky", randint(230, 310), 15)
